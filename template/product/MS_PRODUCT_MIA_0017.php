<?php                                                                        
    ...
    if (!isset($_SESSION['size_brand'])) {
        $_SESSION['size_brand'] = array();
    }

    if (!isset($_SESSION['material_brand'])) {
        $_SESSION['material_brand'] = array();
    }

    if (!isset($_SESSION['laptop_brand'])) {
    ...
    if (!isset($_SESSION['link'])) {
        $_SESSION['link'] = $_GET['page'].$_GET['search'];
    } else {
        if ($_SESSION['link'] != $_GET['page'].$_GET['search']) {
            // $_SESSION['brand'] = array();
            $_SESSION['size_brand'] = array();
            $_SESSION['material_brand'] = array();
            $_SESSION['laptop_brand'] = array();
...
////////////////////
    ...
    if (isset($_GET['slug']) && $_GET['slug'] != '') {
        ...
        $rows = $action_product->getProductList_byMultiLevel_orderProductId_brand($rowCat['productcat_id'],'desc',$trang,40,$slug);//var_dump($rows);
    }else{
...
<div class="gb-page-sanpham_ruouvang">
    <div class="container">
        <div class="row-fix">
            <div class="col-md-3">
                <?php include DIR_SIDEBAR."MS_SIDEBAR_MIA_0016.php";?>
            </div>
            <div class="col-md-9">
                ...
                <div style="text-align: center;"><?= $rows['paging'] ?>

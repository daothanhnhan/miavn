...
public function getProductList_byMultiLevel_orderProductId_brand($idProductCat,$order_productId,$trang, $limit, $page){
    ...
    if ($idProductCat != "" && $nameColIdProductCat != "") {
        ...
        $cond = "";
        $brand = str_replace("-", ",", $_GET['search']);
        $where = "where product_brand in ($brand)";
        ...
        if (!empty($_SESSION['price'])) {
            ...
            foreach ($_SESSION['price'] as $item) {
                ...
                $price2 = $price[1];
                if ($price2 != 0) {
                    $where .= " OR product_price between $price1 and $price2";
                } else {
                    $where .= " OR product_price > $price1";
                }
            }
            $where .= ")";
        }
    ...
    if ($trang != "" && $limit != "") {
        ...
        $config = array(
            ...
            'link_full'=> $slug != '' ? $slug.'/{trang}/'.$_GET['search'] : '/'.$page.'/{trang}/'.$_GET['search'],

            'link_first'=> $slug != '' ? $slug : '/'.$page.'/1/'.$_GET['search'],
...
/*----- liệt kê danh sách product đa cấp dựa vào Product Cat Id cao nhất -------*/
public function getProductList_byMultiLevel_orderProductId_hot($idProductCat,$order_productId,$trang, $limit, $page){
...
/*----- liệt kê danh sách product đa cấp dựa vào Product Cat Id cao nhất -------*/
public function getProductList_byMultiLevel_orderProductId_loc($idProductCat,$order_productId,$trang, $limit, $page){
    ...
    if (empty($_SESSION['brand'])) {
        ...
        foreach ($_SESSION['brand'] as $item) {
            $brand = $item;
            $whereClause .= " OR product_brand = $brand";
        }
        $whereClause .= ")";
    }
    ...
    if (empty($_SESSION['size'])) {
        ...
        foreach ($_SESSION['size'] as $item) {
            // $brand = $item;
            $whereClause .= " OR product_size = $item";
        }
        $whereClause .= ")";
    }
    ...
    if (empty($_SESSION['material'])) {
        ...
        foreach ($_SESSION['material'] as $item) {
            // $brand = $item;
            $whereClause .= " OR product_material = $item";
        }
        $whereClause .= ")";
    }
    ...
    if (empty($_SESSION['laptop'])) {
        ...
        foreach ($_SESSION['laptop'] as $item) {
            // $brand = $item;
            $whereClause .= " OR product_laptop = $item";
        }
        $whereClause .= ")";
    }
    ...
    if (empty($_SESSION['special'])) {
        ...
        foreach ($_SESSION['special'] as $item) {
            // $brand = $item;
            $whereClause .= " OR product_special = $item";
        }
        $whereClause .= ")";
    }
    ...
    if (empty($_SESSION['price'])) {
        ...
        foreach ($_SESSION['price'] as $item) {
            ...
            $price2 = $price[1];
            if ($price2 != 0) {
                $whereClause .= " OR product_price between $price1 and $price2";
            } else {
                $whereClause .= " OR product_price > $price1";
            }
        }
        $whereClause .= ")";
    }
    ...
    if (empty($_SESSION['color'])) {
        ...
        foreach ($_SESSION['color'] as $item) {
            // $brand = $item;
            $whereClause .= " OR product_color = $item";
        }
        $whereClause .= ")";
    }
...
public function listSameBrand ($id, $limit) {
    global $conn_vn;
    $sql = "SELECT * FROM product WHERE product_brand = $id LIMIT $limit";
    $result = mysqli_query($conn_vn, $sql);
    $rows = array();
    $num = mysqli_num_rows($result);
    if ($num > 0) {
...
public function listSameSize ($id, $limit) {
    global $conn_vn;
    $sql = "SELECT * FROM product WHERE product_size = $id LIMIT $limit";
    $result = mysqli_query($conn_vn, $sql);
    $rows = array();
    $num = mysqli_num_rows($result);
    if ($num > 0) {
...
public function sidebarListSize () {
    global $conn_vn;
    ...
    $size_id = array();
    while ($row = mysqli_fetch_assoc($result)) {
          if (!in_array($row['size_name'], $size_name)) {
                $size_name[] = $row['size_name'];
                $size_id[] = $row['size_id'];
          } else {
                $key = array_search($row['size_name'], $size_name);
                $size_id[$key] .= "-".$row['size_id'];
          }
    }
    $size = array();
    foreach ($size_name as $key => $item) {
          $size[] = array(
                'name' => $size_name[$key],
                'id'   => $size_id[$key]
          );
...
public function sidebarListMaterial () {
    global $conn_vn;
    $sql = "SELECT * FROM material";
    $result = mysqli_query($conn_vn, $sql);
    $chat_lieu_name = array();
    $chat_lieu_id = array();
    while ($row = mysqli_fetch_assoc($result)) {
          if (!in_array($row['name'], $chat_lieu_name)) {
                $chat_lieu_name[] = $row['name'];
                $chat_lieu_id[] = $row['id'];
          } else {
                $key = array_search($row['name'], $chat_lieu_name);
                $chat_lieu_id[$key] .= "-".$row['id'];
          }
    }
    $chat_lieu = array();
    foreach ($chat_lieu_name as $key => $item) {
          $chat_lieu[] = array(
                'name' => $chat_lieu_name[$key],
                'id'   => $chat_lieu_id[$key]
          );
    }
    return $chat_lieu;
...
public function sidebarListLaptop () {
    global $conn_vn;
    $sql = "SELECT * FROM container_laptop";
    $result = mysqli_query($conn_vn, $sql);
    $chat_lieu_name = array();
    $chat_lieu_id = array();
    while ($row = mysqli_fetch_assoc($result)) {
